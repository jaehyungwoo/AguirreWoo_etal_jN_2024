function [outVals, subjectLabels] = figure_days_around_rev(block_output, taskType, MET, group_colors, font_size)
outVals = struct;
    ...
    for g = 1:numel(lesion_groups)
        ...
        for i = 1:numel(Revs_to_plot)
            ...
            if perform_sigtest
                ...
                [pval, cohensD, asterisk] = two_sample_T_test(Dat1_before(:), Dat2_before(:), 1, 'both');
                text(0.25, 1.0, [asterisk+"\it{p}=\rm"+num2str(pval,3)],'Units','normalized','HorizontalAlignment','center','VerticalAlignment','middle','FontSize',font_size-4);
                
                % after rev
                Dat1_after = DataToPlot.CNO1(:,numDays+1:end);
                Dat2_after = DataToPlot.VEH1(:,numDays+1:end);
                [pval, cohensD, asterisk] = two_sample_T_test(Dat1_after(:), Dat2_after(:), 1, 'both');
                text(0.75, 1.0, [asterisk+"\it{p}=\rm"+num2str(pval,3)],'Units','normalized','HorizontalAlignment','center','VerticalAlignment','middle','FontSize',font_size-4);
                
                % within-Group change:
                [pval, cohensD, asterisk] = two_sample_T_test(Dat1_before(:), Dat1_after(:), 1, 'both');
                text(0.5, 1.0, [asterisk],'Units','normalized','HorizontalAlignment','left','VerticalAlignment','middle','FontSize',font_size-4,'Color',group_colors{1});
                [pval, cohensD, asterisk] = two_sample_T_test(Dat2_before(:), Dat2_after(:), 1, 'both');
                text(0.5, 0.9, [asterisk],'Units','normalized','HorizontalAlignment','left','VerticalAlignment','middle','FontSize',font_size-4,'Color',group_colors{2});
            end
